// Alle database-interacties voor de prompts

import db from "./db.js";

// Haalt alle prompts op
export async function getAllPrompts() {
  return db.all("SELECT * FROM prompts ORDER BY created_at DESC");
}

// Haalt een specifieke prompt
...
// Werkt een prompt bij
export async function updatePrompt(id, { title, content }) {
  return db.run("UPDATE prompts SET title=?, content=? WHERE id=?", [
    title,
    content,
    id,
  ]);
...
// Verwijdert een specifieke prompt
export async function deletePrompt(id) {
  return db.run("DELETE FROM prompts WHERE id = ?", [id]);
}

export async function saveAssistantInfo(
  promptId,
...
export async function addPromptFiles(promptId, files) {
  const row = await db.get("SELECT file_ids FROM prompts WHERE id=?", [
    promptId,
  ]);
  let arr = [];
  try {
    arr = row?.file_ids ? JSON.parse(row.file_ids) : [];
  } catch {}
  const map = new Map(arr.map((f) => [f.id, f]));
  for (const f of files) map.set(f.id, { id: f.id, name: f.name || f.id });
  await db.run("UPDATE prompts SET file_ids=? WHERE id=?", [
