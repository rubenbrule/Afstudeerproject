...
function getApis() {
  const a = openai;
  const assistants = a.assistants ?? (a.beta && a.beta.assistants) ?? null;
  const vectorStores =
    a.vectorStores ?? (a.beta && a.beta.vector_stores) ?? null;
  const threads = a.threads ?? (a.beta && a.beta.threads) ?? null;
  return { assistants, vectorStores, threads };
}

// Check of de assistant de file_search tool heeft en de files gekoppeld zijn
export async function ensureAssistantHasVectorStore(
  assistantId,
  vectorStoreId
) {
  ...
  if (!assistants) {
    throw new Error(
      "OpenAI SDK: assistants-API niet beschikbaar (upgrade openai package)."
    );
  }

  const a = await assistants.retrieve(assistantId);

  // file_search tool aanzetten
...
export async function waitVectorStoreReady(
  vectorStoreId,
  ...
  if (!vectorStores) {
    throw new Error(
      "OpenAI SDK: vectorStores-API niet beschikbaar (upgrade openai package)."
    );
  }
  ...
  while (true) {
    ...
    if (failed)
      throw new Error(
        "Vector store: één of meer files zijn niet goed geïndexeerd (status=failed)."
      );
...
export async function runAndWait({
  ...
  if (!threads) {
    throw new Error(
      "OpenAI SDK: threads-API niet beschikbaar (upgrade openai package)."
    );
  }
  ...
  while (true) {
    ...
    if (run.status === "requires_action") {
      console.warn(
        "Run requires_action (geen outputs aangeleverd):",
        JSON.stringify(run.required_action, null, 2)
      );
    }
