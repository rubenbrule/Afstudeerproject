...
export default function PromptBeheer() {
  ...
  // Wanneer selectedPrompt wijzigt: gekoppelde files ophalen of resetten
  useEffect(() => {
  ...
  // Sluit de popup met de esc-toets
  useEffect(() => {
    function onKey(e) {
  ...
  // Gesorteerde weergave (default, oplopend (a-z) of aflopend (z-a))
  const sortedPrompts = useMemo(() => {
    ...
    if (sortBy === "az") {
      ...
      arr.sort((a, b) =>
        (b.title || "").localeCompare(a.title || "", "nl", {
          sensitivity: "base",
        })
      );
    }
  ...
  async function handleAddPrompt(e) {
    ...
    try {
      ...
      alert("Fout bij aanmaken van prompt");
    }
  }
  ...
  return (
    <div className="flex gap-4">
        ...
        <ul className="grid gap-2">
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {hasFiles && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-700">
                            {/* Paperclip met daarbij hoeveel files er gelinkt zijn */}
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 24 24"
                              fill="none"
                      ...
                      {/* Korte preview van de prompt */}
                      {preview && (
                        <p className="mt-0.5 line-clamp-1 text-sm text-gray-500">
                          {preview}
                          {(prompt.description ?? prompt.content ?? "").length >
                          120
                            ? "…"
                            : ""}
                        </p>
          ...
          {sortedPrompts.length === 0 && (
            <li className="rounded-2xl border border-dashed border-gray-300 p-6 text-center text-sm text-gray-500">
              Nog geen prompts — klik op <span className="font-medium">+</span>{" "}
              om je eerste prompt aan te maken.
            </li>
          )}
        </ul>
      </div>
      ...
      {/* Popup voor nieuwe prompt aanmaken */}
      {newPromptOpen && (
            ...
            <form
              onSubmit={async (e) => {
                ...
                try {
                  ...
                  alert(
                    "Prompt aangemaakt" +
                      (createFiles.length ? " + bestanden geüpload" : "")
                  );
                } catch (err) {
                  console.error(err);
              ...
              <div className="space-y-2">
                ...
                <div className="mt-3 rounded-md border border-gray-300 bg-gray-50 p-3 text-sm text-gray-700">
                  ...
                  <p className="mt-3 text-xs text-gray-600">
                    Afbeeldingen, audio, video en zip-bestanden worden niet
                    ondersteund door OpenAI.
                  </p>
                </div>
